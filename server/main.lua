...
local function SetupCharacters(source)
    ...
    if result then
        ...
        for i = 1, characterCount, 1 do
            ...
            local id = tonumber(string.sub(v.identifier, #PREFIX+1, string.find(v.identifier, ":")-1)) --[[@as number]]
...
MySQL.ready(function()
    ...
    if DB_COLUMNS then
        ...
        if next(columns) then
            ...
            queries[1] = {query = "SET FOREIGN_KEY_CHECKS = 0"}

            for k, v in pairs(columns) do
                queries[#queries+1] = {query = query:format(k, v, length)}
            end

            queries[#queries+1] = {query = "SET FOREIGN_KEY_CHECKS = 1"}
